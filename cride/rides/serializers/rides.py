...
# TODO utilizar hiddenfield para conseguir el request.user
class JoinRideSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if ride.departure_date <= timezone.now():
            raise serializers.ValidationError('You can not join this ride now')

        if ride.available_seats < 1:
            raise serializers.ValidationError('Ride is already full!')

        if Ride.objects.filter(passengers__id=self.context['user'].id):
...
class EndRideSerializer(serializers.ModelSerializer):
    ...
    def validate_current_time(self, value):
        ...
        if value <= self.instance.departure_date:
